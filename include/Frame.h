...
public:
    ...
    // Constructor for RGB-MonoDepth cameras.
    Frame(const cv::Mat &imGray, const cv::Mat &imRGB, const double &timeStamp, std::shared_ptr<DepthEstimator> depthEstimator, ORBextractor* extractor,ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, GeometricCamera* pCamera,Frame* pPrevF = static_cast<Frame*>(NULL), const IMU::Calib &ImuCalib = IMU::Calib());
